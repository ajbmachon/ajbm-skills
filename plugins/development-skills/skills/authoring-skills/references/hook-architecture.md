...
Deep dive into how Claude Code hooks work internally.
...
The hook cannot detect when the skill is *actually* invoked - it just blocks once per session per skill. Trust that Claude follows the instruction.
...
1. **Loading skill-rules.json** (every execution)
   - Future: Cache in memory, reload only when changed
