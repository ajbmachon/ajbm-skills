...
**Other thinking modes don't call Science. They implement it.**

This document defines the Science Protocol — the interface that modes embody when they apply systematic, evidence-based iteration to their domain.

---

## The Core Insight

Science is not a service that other modes invoke. It's a **protocol they implement**.
...
|-------|-------------|----------|
| **Service (Wrong)** | Development calls Science.analyze() | High — Science must know all modes |
| **Protocol (Right)** | Development implements ScienceProtocol | Low — Modes are independently compliant |
...
## How Modes Implement Science

Modes don't import Science. They declare compliance through structure and behavior.
...
- **Multi-judge panels** — reduces model quirks
...
mapping:
  ...
  hypothesize: Attack vectors (multiple agents generate them)
...
Modes can declare compliance in their SKILL.md frontmatter:
...
1. **Cross-domain problems** — The problem spans multiple modes
2. **Explicit experimentation requests** — "Try 3 approaches and compare"
3. **Novel problems** — No existing mode pattern fits
...
└─────────────────────────────────────────┘
           │
           ├──► Goal: Research helps define success criteria
           │
           ├──► Observe: Research gathers context
           │
           ├──► Hypothesize: Council generates approaches
           │
           ├──► Experiment: Worktrees run parallel implementations
           │
           ├──► Measure: Evals compares approaches
           │
...
### What Science Provides to Other Modes

| To Mode | Science Provides |
|---------|------------------|
...
### What Modes Provide to Science

| From Mode | Mode Provides |
|-----------|---------------|
...
| **Meso** | Hours-Days | Explicit when stuck | Working notes |
| **Macro** | Weeks-Months | Formal documentation | Global registry |
| **Meta** | Months-Years | Constitutional | Versioning |
...
The Science mode MUST be able to improve its own methodology using the scientific method:
...
2. **Delegate to specialist** — Invoke an evals-focused tool or workflow
3. **Use eval infrastructure** — Don't reinvent position swapping, multi-judge, etc.
4. **Accept eval output** — Scores, statistics, recommendations

### Evals → Science

When prompt evaluation gets stuck or needs broader framing:
...
### When Science Orchestrates Across Modes
...
└─────────────────────────────────────────┘
           │
           ├──► Goal: What does "better" mean? (DefineGoal)
           │
           ...
           │
           ├──► Experiment: Parallel experiments via appropriate modes
           │
           ├──► Measure: Aggregate results from each mode
           │
           └──► Iterate: Pick winner, next cycle
```

Science orchestrates; specialist modes execute.

---

## Summary

**Science is not a mode you call. It's a pattern you embody.**

- Modes implement the protocol independently
