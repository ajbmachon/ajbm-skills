...
- *Maps to: Each pass tests emerging understanding against a new angle, revising until the requirement model coheres*
...
- *Maps to: Multiple reasoning paths = multiple lenses on the same requirement extraction*
...
- *Maps to: Combining multiple requirement extraction passes = ensemble of requirements*
...
- *Maps to: Each iteration adds precision to success criteria*
...
- *Maps to: The direct inspiration for the 8 lenses*
...
The CS technique searches the SAME TREE deeper each time. This technique searches the SAME PROBLEM from DIFFERENT ANGLES each time. Related in spirit (both benefit from re-examination), fundamentally different in mechanism.
