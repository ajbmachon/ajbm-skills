...
**Output:** Quality-of-experience criteria that elevate from functional to delightful.
...
## Time-Based Lens Selection

| Time Budget | Lenses Used | Which Ones |
|-------------|-------------|------------|
| Skip | 0 | Skip iterative-depth entirely |
| <30s | 2 | Literal + Failure |
| <2min | 4 | Literal + Stakeholder + Failure + Experiential |
| <5min | 8 | All 8 lenses |

At the shortest budget, the two most commonly productive lenses (surface requirements + what goes wrong) run as brief internal thought exercises — not spawned agents.

At medium budget, 4 lenses run. These can be parallelized as 2 pairs of background agents.

At long budget, all 8 lenses run. These can be parallelized as 4 pairs or 8 individual agents for maximum coverage.
...
For specialized domains, the calling workflow can override lens selection:
