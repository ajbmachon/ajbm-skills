...
Adversarial analysis using parallel agent deployment. Breaks arguments into atomic components, attacks from up to 32 different expert perspectives simultaneously, synthesizes findings, and produces the ultimate counter-argument.
...
Break the argument into 24 atomic claims using structured decomposition.

### PHASE 2: PARALLEL ANALYSIS
Launch up to 32 specialized agents (8 each: engineers, architects, pentesters, interns) with unique personalities to examine BOTH strengths AND weaknesses from their perspective.
...
```
→ first-principles/workflows/Deconstruct.md on the argument
```
...
### Launch Protocol
Deploy up to 32 agents in a SINGLE message with multiple Task tool calls. Each agent receives:
...
| EN-4 | **The Historical Pattern Matcher** — Has seen every failure mode. | "We tried this before and here's what happened." |
...
```
→ first-principles/workflows/Challenge.md on all stated constraints and assumptions
```
...
```
□ PHASE 1: Decomposition
  □ Invoke first-principles Deconstruct on the argument
...
□ PHASE 2: Parallel Analysis
  □ Prepare up to 32 agent prompts with unique personalities
...
□ PHASE 5: Counter-Argument
  □ Invoke first-principles Challenge on all constraints
...
- Ability to launch many agents simultaneously
- Synthesis capability to process multiple agent outputs
- **First principles mode** for Deconstruct (Phase 1) and Challenge (Phase 5)

**Pairs well with:**
- `first-principles/workflows/Deconstruct.md` — breaks argument into fundamental parts (Phase 1)
- `first-principles/workflows/Challenge.md` — classifies constraints as HARD/SOFT/ASSUMPTION (Phase 5)
- Research for finding counterexamples and precedents

**Time estimate:**
- Phase 1: 2-3 minutes (decomposition)
- Phase 2: 3-5 minutes (parallel execution)
- Phase 3: 2-3 minutes (synthesis)
- Phase 4: 2-3 minutes (steelman)
- Phase 5: 2-3 minutes (counter-argument)
